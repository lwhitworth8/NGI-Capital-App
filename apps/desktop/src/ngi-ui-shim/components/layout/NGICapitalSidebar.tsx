...
export default function NGICapitalSidebar({ brand, items, activePath, LinkComponent, user, onSettings, onSignOut }: Props) {
  return (
    <aside className="w-64 bg-card border-r border-border h-screen sticky top-0 flex flex-col">
      <div className="px-4 py-4 border-b border-border font-bold text-lg">{brand}</div>
      {user && (
        <div className="px-4 py-3 flex items-center gap-3 border-b border-border">
          <div className="w-9 h-9 rounded-full flex items-center justify-center text-white" style={{ backgroundColor: user.color || '#1f2937' }}>
            {user.photoUrl ? <img src={user.photoUrl} alt="avatar" className="w-9 h-9 rounded-full object-cover" /> : (user.initials || 'U')}
          </div>
