...
export function ApplicationModal({ 
  ...
  const validateForm = () => {
    ...
    questions.forEach((q: any) => {
      const key = 'q_' + q.id
      if (q.is_required && !(applicationData[key] || '').trim()) {
              ...
              <div className="p-8 space-y-6">
                {submitted && (
                  <div className="space-y-3">
                    <div className="text-lg font-semibold">Thanks for applying!</div>
                    <p className="text-sm text-muted-foreground">We’ve received your application. Please watch your email for next steps and a potential interview invitation.</p>
                  </div>
                )}
