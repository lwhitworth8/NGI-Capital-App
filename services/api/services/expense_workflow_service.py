...
from services.api.models_expenses_payroll import ExpenseReport, ExpenseLine, EmployeePayrollInfo
...
# JE automation removed; will be reintroduced with new system
...
class ExpenseWorkflowService:
    ...
    async def approve(
        ...
        # JE creation disabled pending redesign
    ...
    async def _create_expense_je_unified(self, report: ExpenseReport) -> Dict:
        """Create JE using unified service"""
        try:
            # Get expense lines
            ...
            for line in expense_lines:
                je_lines.append({
                    ...
                    'amount': line.amount,
                    'type': 'debit',
                    'description': f"{line.category} - {line.description[:50]}"
                })
            
            # Add cash credit line
            ...
            je_lines.append({
                ...
                'amount': report.reimbursable_amount,
                'type': 'credit',
                'description': f"Reimbursement to {report.employee_name}"
            })
            
            # JE creation disabled
            return {"success": False, "message": "JE disabled"}
