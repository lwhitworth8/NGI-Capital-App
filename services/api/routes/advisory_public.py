...
from services.api.agent_client import start_agent_run, ensure_agent_tables
from services.api.clerk_auth import verify_clerk_jwt as _verify_clerk_jwt, verify_clerk_session_cookie as _verify_clerk_session
...
async def public_projects(
    ...
    # Use raw sqlite3 for consistent positional tuples across runtimes
    try:
        con = sqlite3.connect(get_database_path())
        cur = con.cursor()
        rows = cur.execute(sql, params).fetchall()
    except Exception:
        # Fallback to SQLAlchemy execution if sqlite3 fails
    ...
    out = []
    for r in rows:
        # r is a sqlite3 row tuple here; fall back to tuple conversion otherwise
        vals = tuple(r)
        ...
        out.append({
            ...
            "client_name": vals[2],
            "summary": vals[3],
            "hero_image_url": vals[4],
            "gallery_urls": _try_json(vals[5]),
            "showcase_pdf_url": vals[6],
            "tags": _try_json(vals[7]),
            "partner_badges": _try_json(vals[8]),
            "backer_badges": _try_json(vals[9]),
            "start_date": vals[10],
            "end_date": vals[11],
            "allow_applications": int(vals[12] or 0),
            "status": vals[13],
            "mode": vals[14],
            "location_text": vals[15],
            "team_size": vals[16],
            "default_hourly_rate": vals[17],
            "pay_currency": vals[18],
            "applied_count": int(vals[19] if len(vals) > 19 and vals[19] is not None else 0),
        })
    return out
...
async def public_project_detail(pid: int, db: Session = Depends(get_db)):
    ...
    row = db.execute(sa_text(
        "SELECT id, project_name, client_name, summary, description, hero_image_url, gallery_urls, tags, partner_badges, backer_badges, allow_applications, "
        "coffeechat_calendly, status, mode, location_text, start_date, end_date, duration_weeks, commitment_hours_per_week, team_size, team_requirements, team_composition, showcase_pdf_url, applications_close_date "
...
def _ensure_student_profile_cols(db: Session) -> None:
    ...
    add_cols = (
        ...
        # New: avatar styling
        "ALTER TABLE advisory_students ADD COLUMN profile_color TEXT",
        "ALTER TABLE advisory_students ADD COLUMN profile_photo_url TEXT",
    )
    for sql in add_cols:
        try:
...
async def get_profile(request: Request, db: Session = Depends(get_db)):
    ...
    if not email or not _check_domain(email):
        raise HTTPException(status_code=403, detail="Student email with allowed domain required")
    row = db.execute(sa_text(
        "SELECT id, first_name, last_name, email, school, program, grad_year, phone, linkedin_url, gpa, location, resume_url, theme, learning_notify, uc_investments_academy, created_at, updated_at, profile_color, profile_photo_url "
    ...
    return {
        ...
        "created_at": row[15],
        "updated_at": row[16],
        "profile_color": row[17],
        "profile_photo_url": row[18],
    }
...
async def update_profile(payload: Dict[str, Any], request: Request, db: Session = Depends(get_db)):
    ...
    # profile_color (simple text)
    if "profile_color" in payload:
        col = str(payload.get("profile_color") or "").strip()
        fields.append("profile_color = :pc"); params["pc"] = (col or None)
...
async def upload_profile_photo(request: Request, file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    out_path = base / f"profile-{ts}.{ext}"
    with out_path.open("wb") as out:
        contents = await file.read()
        out.write(contents)
    rel_url = str(out_path.as_posix())
    db.execute(sa_text("UPDATE advisory_students SET profile_photo_url = :u, updated_at = datetime('now') WHERE lower(email) = :em"), {"u": rel_url, "em": email.lower()})
    db.commit()
    return {"profile_photo_url": rel_url}


@router.delete("/profile/photo")
async def delete_profile_photo(request: Request, db: Session = Depends(get_db)):
    _ensure_tables(db)
    _ensure_student_profile_cols(db)
    email = _extract_student_email(request)
    if not email or not _check_domain(email):
        raise HTTPException(status_code=403, detail="Student email with allowed domain required")
    row = db.execute(sa_text("SELECT profile_photo_url FROM advisory_students WHERE lower(email) = :em"), {"em": email.lower()}).fetchone()
    if row and row[0]:
        try:
            path = str(row[0])
            ...
            pass
    db.execute(sa_text("UPDATE advisory_students SET profile_photo_url = NULL, updated_at = datetime('now') WHERE lower(email) = :em"), {"em": email.lower()})
    db.commit()
    return {"message": "removed"}


@router.post("/profile/photo/delete")
async def delete_profile_photo_post(request: Request, db: Session = Depends(get_db)):
    return await delete_profile_photo(request, db)
