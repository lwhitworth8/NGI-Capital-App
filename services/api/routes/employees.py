...
def _ensure_hr_schema(db: Session) -> None:
    ...
    # Progressive schema: ordering support for Kanban
    _add_column_if_missing(db, "projects", "sort_order", "INTEGER")
    ...
    # Clean up auto-created teams that shouldn't exist
    _cleanup_auto_created_teams(db)


def _ensure_default_teams(db: Session, entity_id: int) -> None:
    # Teams are now entity-specific and only created when explicitly requested
    # No automatic creation of default teams
    pass


def _cleanup_auto_created_teams(db: Session) -> None:
    """Remove automatically created default teams that shouldn't exist"""
    # Remove default teams that were auto-created
    db.execute(
        sa_text("DELETE FROM teams WHERE name IN ('Executive', 'Board') AND description = ''"),
    )
    db.commit()


def _create_advisory_project_teams(db: Session, entity_id: int) -> None:
    """Create teams for NGI Capital Advisory based on active projects"""
    # Check if this is NGI Capital Advisory entity
    # For now, we'll create teams for any entity that requests it
    # In the future, this could be entity-specific logic
    
    # Get active projects from advisory_projects table
    try:
        projects = db.execute(
            sa_text("""
                SELECT id, project_name, client_name, status 
                FROM advisory_projects 
                WHERE status = 'active' AND entity_id = :eid
        ...
        for project in projects:
            ...
            if not existing:
                ...
                db.execute(
                    sa_text("""
                        INSERT INTO teams (entity_id, name, description, type, active, created_at, updated_at)
                        VALUES (:eid, :name, :desc, 'project', 1, datetime('now'), datetime('now'))
                    """),
                    {
                        "eid": entity_id,
                        "name": team_name,
                        "desc": f"Team for {project_name} project with {client_name}"
                    }
                )
        
        db.commit()
    except Exception as e:
        # If advisory_projects table doesn't exist, just continue
        pass
...
async def list_teams(
    ...
    # Create advisory project teams if this is NGI Capital Advisory
    _create_advisory_project_teams(db, eid)
    
    # Get teams with member count from team_memberships
    rows = db.execute(
        sa_text("""
            SELECT t.id, t.name, t.description, COALESCE(t.active,1), t.type, t.lead_employee_id,
                   COUNT(tm.employee_id) as member_count
            FROM teams t
            LEFT JOIN team_memberships tm ON t.id = tm.team_id
            WHERE t.entity_id = :e
            GROUP BY t.id, t.name, t.description, t.active, t.type, t.lead_employee_id
            ORDER BY t.name
        """),
        {"e": eid},
    ).fetchall()
    ...
    return [
        {
            ...
            "employee_count": r[6] or 0,  # Add member count
        }
        for r in rows
    ]
...
async def add_team_member(
    ...
    # Get team name to set appropriate default role
    team_row = db.execute(
        sa_text("SELECT name FROM teams WHERE id = :tid"),
        {"tid": team_id}
    ).fetchone()
    team_name = team_row[0].lower() if team_row else ""
    
    # Set default role based on team name
    default_role = payload.get("role_on_team")
    if not default_role:
        if team_name == "board":
            default_role = "Board Member"
        elif team_name == "executive":
...
async def update_team(
    ...
    team = db.execute(
        sa_text("SELECT id FROM teams WHERE id = :tid"),
        {"tid": team_id}
    ).fetchone()
    ...
    db.execute(
        sa_text("""
            ...
            SET name = :name, description = :desc, type = :type, 
                lead_employee_id = :lead, updated_at = datetime('now')
            WHERE id = :tid
        """),
        {
            "tid": team_id,
...
async def delete_team(
    ...
    db.execute(
        sa_text("DELETE FROM team_memberships WHERE team_id = :tid"),
        {"tid": team_id}
    )
    
    # Delete the team
    db.execute(
        sa_text("DELETE FROM teams WHERE id = :tid"),
        {"tid": team_id}
    )
...
async def list_projects(
    ...
    rows = db.execute(
        sa_text(
            ...
            FROM projects
            WHERE entity_id = :e
            ORDER BY CASE WHEN sort_order IS NULL THEN 1000000 ELSE sort_order END ASC, id DESC
            """
        ),
        {"e": eid},
    ).fetchall()
...
async def create_project(
    ...
    if not entity_id or not name:
        raise HTTPException(status_code=422, detail="Missing entity_id or name")
    # Determine initial sort_order at end of the target status column
    target_status = (payload.get("status") or "Exploring").strip() or "Exploring"
    row = db.execute(
        sa_text("SELECT MAX(sort_order) FROM projects WHERE entity_id = :e AND status = :s"),
        {"e": entity_id, "s": target_status},
    ).fetchone()
    next_order = (row[0] or 0) + 1
    db.execute(
        sa_text(
            "INSERT INTO projects (entity_id, name, description, status, is_active, sort_order) VALUES (:e,:n,:d,:s,1,:o)"
        ),
        {
            "e": entity_id,
            "n": name,
...
async def get_project(
    ...
    _ensure_hr_schema(db)
    row = db.execute(
        sa_text("SELECT id, entity_id, name, description, status, is_active FROM projects WHERE id = :id"),
        {"id": int(project_id)},
    ).fetchone()
    if not row:
...
async def update_project(
    ...
    """Update project fields (name, description, status, is_active)."""
    _ensure_hr_schema(db)
    # Validate exists
    exists = db.execute(sa_text("SELECT 1 FROM projects WHERE id = :id"), {"id": int(project_id)}).fetchone()
    if not exists:
        raise HTTPException(status_code=404, detail="Project not found")

    allowed = {"name", "description", "status", "is_active", "sort_order"}
    sets: list[str] = []
    params: Dict[str, Any] = {"id": int(project_id)}
    for k, v in (payload or {}).items():
        if k in allowed:
            ...
            params[col] = v
    if not sets:
        return {"id": int(project_id), "updated": 0}
    sql = "UPDATE projects SET " + ", ".join(sets) + ", is_active = coalesce(is_active,1) WHERE id = :id"
    db.execute(sa_text(sql), params)
    db.commit()
    return {"id": int(project_id), "updated": 1}
...
async def reorder_projects(
    ...
    """
    Reorder projects within columns and update their status.
    Payload shape:
    {
      "entity_id": 1,
      "columns": {
    ...
    if not isinstance(columns, dict):
        raise HTTPException(status_code=422, detail="columns mapping required")

    updated = 0
    for status in ("Exploring", "Building", "Launched"):
        ...
        for idx, pid in enumerate(ids):
            ...
            db.execute(
                sa_text(
                    "UPDATE projects SET status = :s, sort_order = :o WHERE id = :id AND entity_id = :e"
                ),
                {"s": status, "o": idx + 1, "id": pid_int, "e": entity_id},
            )
            updated += 1
    db.commit()
...
async def list_employees(
    ...
    for r in rows:
        ...
        # Get team memberships (multiple teams)
        team_memberships = db.execute(
            sa_text(
                "SELECT tm.team_id, t.name FROM team_memberships tm JOIN teams t ON t.id = tm.team_id WHERE tm.employee_id = :eid"
            ),
            {"eid": r[0]},
        ).fetchall()
        
        # Extract team_ids and team_names arrays
        team_ids = [tm[0] for tm in team_memberships]
        team_names = [tm[1] for tm in team_memberships]
        ...
        result.append(
            {
                ...
                "start_date": r[8],
                "end_date": r[9],
                "team_id": r[10],  # Keep for backward compatibility
                "team_name": r[11],  # Keep for backward compatibility
                "compensation_type": '',  # Default values for now
                "hourly_rate": 0,
                "annual_salary": 0,
                "team_ids": team_ids,  # New: array of team IDs
                "team_names": team_names,  # New: array of team names
...
async def create_employee(
    ...
    # Add compensation - always save, even if 0
    if "compensation_type" in payload:
        insert_cols.append("compensation_type")
        insert_vals.append(":ct")
        insert_params["ct"] = payload.get("compensation_type")
    if "hourly_rate" in payload:
        insert_cols.append("hourly_rate")
        insert_vals.append(":hr")
        insert_params["hr"] = float(payload.get("hourly_rate") or 0)
    if "annual_salary" in payload:
        insert_cols.append("annual_salary")
        insert_vals.append(":sal")
        insert_params["sal"] = float(payload.get("annual_salary") or 0)
    ...
    # Create team memberships - handle both team_ids array and single team_id
    team_ids_to_assign = []
    
    # Handle team_ids array (new multi-team functionality)
    if payload.get("team_ids") and isinstance(payload.get("team_ids"), list):
        team_ids_to_assign = [int(tid) for tid in payload.get("team_ids") if tid]
    # Fallback to single team_id for backward compatibility
    elif team_id:
        team_ids_to_assign = [team_id]
    
    # Create team memberships for all assigned teams
    for tid in team_ids_to_assign:
        try:
            db.execute(
                sa_text("""
                    INSERT OR IGNORE INTO team_memberships (team_id, employee_id, role_on_team, start_date, allocation_pct) 
                    VALUES (:tid, :eid, :role, :sd, 100)
                """),
                {
                    "tid": tid,
                    "eid": new_id,
                    "role": "Executive" if classification == "executive" else "Member",
                    "sd": payload.get("start_date") or None,
                }
            )
        except Exception as e:
            print(f"Error creating team membership for team {tid}, employee {new_id}: {e}")
...
async def update_employee(
    ...
    # Handle team assignments separately from other fields
    team_ids_to_assign = []
    if payload.get("team_ids") and isinstance(payload.get("team_ids"), list):
        team_ids_to_assign = [int(tid) for tid in payload.get("team_ids") if tid]
    elif payload.get("team_id"):
        team_ids_to_assign = [int(payload.get("team_id"))]
    
    # Update team memberships if team assignments are provided
    if "team_ids" in payload or "team_id" in payload:  # Only update if team assignment is provided
        # Remove existing team memberships
        try:
            db.execute(
                sa_text("DELETE FROM team_memberships WHERE employee_id = :eid"),
                {"eid": emp_id}
            )
        except Exception as e:
            print(f"Error deleting team memberships for employee {emp_id}: {e}")
        ...
        for tid in team_ids_to_assign:
            try:
                ...
                print(f"Error adding team membership for team {tid}, employee {emp_id}: {e}")
                # Continue with other teams even if one fails
    ...
    fields = [
        ...
        ("end_date", "end_date"),
        ("team_id", "team_id"),  # Keep for backward compatibility
...
async def get_my_profile(
    ...
    """Return current admin's employee profile (by email), if present.
    Falls back gracefully when not found.
    """
    _ensure_hr_schema(db)
    email = str(partner.get("email") or "").strip().lower()
    if not email:
        raise HTTPException(status_code=401, detail="Email not resolved from auth")
    # Prefer row scoped to selected entity (if provided); otherwise fallback to any
    eid = _resolve_entity_id(entity, entity_id)
    params: Dict[str, Any] = {"em": email}
    sql = (
        "SELECT id, coalesce(legal_name, name) as name, email, title, role, classification, status, "
        "       employment_type, profile_color, profile_photo_url, entity_id "
        "FROM employees "
        "WHERE lower(email) = :em AND (is_deleted = 0 OR is_deleted IS NULL) "
    )
    if eid:
    ...
    if not row and eid:
        ...
        row = db.execute(sa_text(
            "SELECT id, coalesce(legal_name, name) as name, email, title, role, classification, status, "
            "       employment_type, profile_color, profile_photo_url, entity_id "
            "FROM employees WHERE lower(email) = :em AND (is_deleted = 0 OR is_deleted IS NULL) "
            "ORDER BY id DESC LIMIT 1"
        ), {"em": email}).fetchone()
    if not row:
        # Not all admins are employees; return minimal identity-only payload
        return {
            "employee_id": None,
            "name": partner.get("name") or email.split("@")[0],
            "email": email,
            "title": None,
            "role": None,
            "status": None,
    ...
    emp_id = int(row[0])
    # Teams for employee, scoped to same entity as the row (or provided eid)
    try:
        eff_entity = int(row[10]) if row[10] is not None else (eid or 0)
    except Exception:
        eff_entity = (eid or 0)
    team_rows = db.execute(sa_text(
        """
        SELECT t.name
        FROM team_memberships tm
        JOIN teams t ON t.id = tm.team_id
        WHERE tm.employee_id = :eid AND (:ten IS NULL OR t.entity_id = :ten)
        ORDER BY t.name
        """
    ), {"eid": emp_id, "ten": (eff_entity if eff_entity else None)}).fetchall()
    team_names = [tr[0] for tr in team_rows]
    return {
        "employee_id": emp_id,
        "name": row[1],
        "email": row[2],
...
async def update_my_profile(
    ...
    """Allow updating limited self-profile fields: full name, profile_color.
    If multiple employee rows share the email, update all rows for consistency.
    """
    _ensure_hr_schema(db)
    email = str(partner.get("email") or "").strip().lower()
    if not email:
    ...
    # Full name maps to both legal_name and name for consistency across modules
    full_name = payload.get("full_name") or payload.get("name") or payload.get("legal_name")
    if isinstance(full_name, str) and full_name.strip():
        sets.append("legal_name = :nm")
        sets.append("name = :nm")
        params["nm"] = full_name.strip()

    # Optional profile color (hex or semantic). Basic validation.
    if "profile_color" in payload:
        col = str(payload.get("profile_color") or "").strip()
        if col and not __import__("re").match(r"^(#?[0-9a-fA-F]{3}|#?[0-9a-fA-F]{6}|[a-zA-Z][a-zA-Z0-9_-]{1,20})$", col):
            raise HTTPException(status_code=422, detail="Invalid color format")
        sets.append("profile_color = :pc"); params["pc"] = (col or None)

    if not sets:
        return {"message": "no changes"}
    sets.append("updated_at = datetime('now')")
    sql = "UPDATE employees SET " + ", ".join(sets) + " WHERE lower(email) = :em AND (is_deleted = 0 OR is_deleted IS NULL)"
    db.execute(sa_text(sql), params)
    db.commit()
    return {"message": "updated"}
...
async def upload_my_profile_photo(
    ...
    """Upload a profile photo and set profile_photo_url for the employee(s) with this email."""
    _ensure_hr_schema(db)
    email = str(partner.get("email") or "").strip().lower()
    if not email:
        raise HTTPException(status_code=401, detail="Email not resolved from auth")
    filename = (file.filename or "profile.jpg").lower()
    ...
    # Store under uploads/employees/{safe_email}/profile-<ts>.<ext>
    safe_email = email.replace("/", "_").replace("\\", "_").replace("@", "_at_")
    out_dir = Path("uploads") / "employees" / safe_email
    out_dir.mkdir(parents=True, exist_ok=True)
    ts = datetime.utcnow().strftime("%Y%m%d%H%M%S%f")
    ext = filename.split(".")[-1]
    out_path = out_dir / f"profile-{ts}.{ext}"
    with out_path.open("wb") as out:
        contents = await file.read()
        out.write(contents)
    rel_url = str(out_path.as_posix())
    db.execute(sa_text(
        "UPDATE employees SET profile_photo_url = :u, updated_at = datetime('now') WHERE lower(email) = :em AND (is_deleted = 0 OR is_deleted IS NULL)"
    ), {"u": rel_url, "em": email})
    db.commit()
    return {"profile_photo_url": rel_url}
...
async def delete_my_profile_photo(
    ...
    """Remove profile photo and clear profile_photo_url.
    Attempts to delete the last uploaded file if it lives under /uploads.
    """
    _ensure_hr_schema(db)
    email = str(partner.get("email") or "").strip().lower()
    if not email:
        raise HTTPException(status_code=401, detail="Email not resolved from auth")
    row = db.execute(sa_text("SELECT profile_photo_url FROM employees WHERE lower(email) = :em AND (is_deleted = 0 OR is_deleted IS NULL) ORDER BY id DESC LIMIT 1"), {"em": email}).fetchone()
    if row and row[0]:
        try:
            path = str(row[0])
            ...
            pass
    db.execute(sa_text("UPDATE employees SET profile_photo_url = NULL, updated_at = datetime('now') WHERE lower(email) = :em"), {"em": email})
    db.commit()
    return {"message": "removed"}


@router.post("/me/profile/photo/delete")
async def delete_my_profile_photo_post(
    partner=Depends(_require_clerk_user),
    db: Session = Depends(get_db),
):
    """Compatibility endpoint for environments or proxies that block DELETE.
    Performs the same logic as DELETE /me/profile/photo.
    """
    return await delete_my_profile_photo(partner=partner, db=db)
...
async def list_timesheets(
    ...
    if employee_id:
        where.append("t.employee_id = :emp")
