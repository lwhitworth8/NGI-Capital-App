...
from ..models_accounting import ChartOfAccounts, AccountingEntity, AccountMappingRule, JournalEntry, JournalEntryLine
...
class XBRLMapRequest(BaseModel):
    xbrl_element_name: str
...
async def get_all_accounts(
    ...
    # Compute posted balances per account in one query
    bal_q = (
        select(
            JournalEntryLine.account_id.label("aid"),
            func.coalesce(func.sum(JournalEntryLine.debit_amount), 0).label("deb"),
            func.coalesce(func.sum(JournalEntryLine.credit_amount), 0).label("cred"),
        )
    ...
    bal_rows = await db.execute(bal_q)
    bal_map = {int(r.aid): (Decimal(str(r.deb or 0)), Decimal(str(r.cred or 0))) for r in bal_rows}

    # Build responses and annotate children + current_balance
    responses = []
    for account in accounts:
        ...
        deb, cred = bal_map.get(int(account.id), (Decimal("0"), Decimal("0")))
        raw = deb - cred  # debit-normal amount
        norm = (str(getattr(account, "normal_balance", "Debit") or "Debit").lower())
        signed = -raw if norm == "credit" else raw
        account_response.current_balance = signed
...
@router.post("/{account_id}/map-xbrl")
async def map_xbrl_for_account(
    account_id: int,
    body: XBRLMapRequest,
    db: AsyncSession = Depends(get_async_db)
):
    """Set XBRL mapping for a COA account and derive ASC topic from taxonomy"""
    # Load account
    res = await db.execute(select(ChartOfAccounts).where(ChartOfAccounts.id == account_id))
    acc = res.scalar_one_or_none()
    if not acc:
        raise HTTPException(status_code=404, detail="Account not found")

    # Resolve element and primary ASC via taxonomy
    svc = get_xbrl_service()
    element = None
    try:
        element = svc.get_element(body.xbrl_element_name)
    except Exception:
        element = None
    if element is None:
        raise HTTPException(status_code=400, detail="XBRL element not found in taxonomy")

    acc.xbrl_element_name = body.xbrl_element_name
    acc.primary_asc_topic = element.get("primary_asc_topic") or acc.primary_asc_topic
    acc.xbrl_is_validated = True
    acc.xbrl_mapping_confidence = Decimal("1.00")
    acc.xbrl_validated_at = get_pst_now()
    ...
    return {
        ...
        "xbrl_element_name": acc.xbrl_element_name,
        "primary_asc_topic": acc.primary_asc_topic,
    }
