...
def _get_status_label(entity: AccountingEntity) -> str:
    ...
    elif entity.entity_status == "pending_conversion":
        parts.append("Pending Conversion")
...
# Support both with and without trailing slash to avoid client redirects
...
async def get_accounting_entities(
    ...
    """
    try:
        # Query active, planned, and pending_conversion entities (show all for dev)
        query = select(AccountingEntity).where(
            AccountingEntity.entity_status.in_(["active", "planned", "pending_conversion"])
        ).order_by(AccountingEntity.entity_name)

        result = await db.execute(query)
        entities = result.scalars().all()

        # Map accounting_entities to response
        response: List[EntityResponse] = [
            EntityResponse(
                id=e.id,
                entity_name=e.entity_name,
                entity_type=e.entity_type,
                ein=getattr(e, 'ein', None),
                entity_status=getattr(e, 'entity_status', 'active'),
                is_available=bool(getattr(e, 'is_available', True)),
                parent_entity_id=getattr(e, 'parent_entity_id', None),
