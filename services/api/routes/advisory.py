...
from services.api.auth_deps import require_clerk_user  # Clerk-only auth dependency
...
def require_ngiadvisory_admin():
    ...
    Note: For v1, Clerk is the sole authentication provider (see require_clerk_user).
    """
    import os as _os
    if str(_os.getenv('OPEN_NON_ACCOUNTING', '0')).strip().lower() in ('1','true','yes'):
        ...
        return _bypass
    from services.api.auth_deps import require_admin as _require_admin  # type: ignore
...
async def get_project(pid: int, admin=Depends(require_ngiadvisory_admin()), db: Session = Depends(get_db)):
    ...
    proj = {
        ...
        "is_public": bool(r[36]), "allow_applications": bool(r[37]),
...
async def update_project(pid: int, payload: Dict[str, Any], request: Request, admin=Depends(require_ngiadvisory_admin()), db: Session = Depends(get_db)):
    ...
    for k in ("client_name","project_name","summary","description","status","mode","location_text","start_date","end_date","duration_weeks","commitment_hours_per_week","project_code","project_lead","contact_email","hero_image_url","apply_cta_text","apply_url","eligibility_notes","notes_internal","team_size","showcase_pdf_url","applications_close_date","default_hourly_rate","pay_currency","compensation_notes"):
        if k in payload:
...
async def list_students(
    ...
    # Debug logging
    print(f"🔍 Students API debug - school: {school}, program: {program}, grad_year_min: {grad_year_min}, grad_year_max: {grad_year_max}")
    
    # Debug: Check what school values exist in database
    try:
        # First check if table exists and has any data
        count_result = db.execute(sa_text("SELECT COUNT(*) FROM advisory_students")).fetchone()
        print(f"🔍 Total students in DB: {count_result[0] if count_result else 0}")
        
        if count_result and count_result[0] > 0:
            school_rows = db.execute(sa_text("SELECT DISTINCT school FROM advisory_students WHERE school IS NOT NULL AND school != ''")).fetchall()
            print(f"🔍 Available schools in DB: {[row[0] for row in school_rows]}")
            
            program_rows = db.execute(sa_text("SELECT DISTINCT program FROM advisory_students WHERE program IS NOT NULL AND program != ''")).fetchall()
            print(f"🔍 Available programs in DB: {[row[0] for row in program_rows]}")
            
            # Show a few sample records
            sample_rows = db.execute(sa_text("SELECT first_name, last_name, school, program FROM advisory_students LIMIT 3")).fetchall()
            print(f"🔍 Sample records: {[(row[0], row[1], row[2], row[3]) for row in sample_rows]}")
        else:
            print("🔍 No students found in database")
    except Exception as e:
        print(f"🔍 Error checking DB values: {e}")
        import traceback
        traceback.print_exc()
    ...
    # School filter (comma-separated list) - use LIKE for partial matching
    if school:
        schools = [s.strip() for s in school.split(',') if s.strip()]
        if schools:
            school_conditions = []
            for i, s in enumerate(schools):
                school_conditions.append(f"lower(school) LIKE :school_{i}")
                params[f"school_{i}"] = f"%{s.lower()}%"
            where.append(f"({' OR '.join(school_conditions)})")
    
    # Program filter (comma-separated list) - use LIKE for partial matching
    if program:
        programs = [p.strip() for p in program.split(',') if p.strip()]
        if programs:
            program_conditions = []
            for i, p in enumerate(programs):
                program_conditions.append(f"lower(program) LIKE :program_{i}")
                params[f"program_{i}"] = f"%{p.lower()}%"
            where.append(f"({' OR '.join(program_conditions)})")
    
    # Graduation year range filters
    if grad_year_min is not None:
        where.append("grad_year >= :grad_year_min")
        params["grad_year_min"] = int(grad_year_min)
    
    if grad_year_max is not None:
        where.append("grad_year <= :grad_year_max")
        params["grad_year_max"] = int(grad_year_max)
