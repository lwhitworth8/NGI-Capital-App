...
from services.api import database_async  # noqa: F401

# Import route modules - using absolute imports for Docker
# NOTE: banking import removed - using accounting_banking instead
from services.api.routes import reports, employees, investor_relations
from services.api.routes import entities as entities_routes
from services.api.routes import accounting_entities as accounting_entities_routes
# accounting_documents temporarily disabled to resolve syntax issues
# from services.api.routes import accounting_documents as accounting_documents_routes
from services.api.routes import accounting_coa as accounting_coa_routes
from services.api.routes import accounting_documents as accounting_documents_routes
from services.api.routes import accounting_journal_entries as accounting_journal_entries_routes
from services.api.routes import accounting_bank_reconciliation as accounting_bank_reconciliation_routes
from services.api.routes import accounting_financial_reporting as accounting_financial_reporting_routes
from services.api.routes import accounting_notes as accounting_notes_routes
# accounting_internal_controls is temporarily disabled (models removed)
# from services.api.routes import accounting_internal_controls as accounting_internal_controls_routes
from services.api.routes import accounting_entity_conversion as accounting_entity_conversion_routes
from services.api.routes import accounting_consolidated_reporting as accounting_consolidated_reporting_routes
from services.api.routes import accounting_period_close as accounting_period_close_routes
from services.api.routes import accounting_ar as accounting_ar_routes
from services.api.routes import accounting_ap as accounting_ap_routes
from services.api.routes import accounting_fixed_assets as accounting_fixed_assets_routes
from services.api.routes import accounting_expenses_payroll as accounting_expenses_payroll_routes
# Legacy banking routes removed; use accounting_bank_reconciliation_routes
from services.api.routes import accounting_reporting as accounting_reporting_routes
from services.api.routes import accounting_tax as accounting_tax_routes
from services.api.routes import accounting_period_close as accounting_period_close_routes
from services.api.routes import advisory as advisory_routes
from services.api.routes import advisory_public as advisory_public_routes
from services.api.routes import coffeechats_internal as coffeechats_internal_routes
from services.api.routes import agents as agents_routes
from services.api.routes import plm as plm_routes
from services.api.routes import chatkit_simple as chatkit_routes
from services.api.routes import project_ai as project_ai_routes
from services.api.routes import mappings as mappings_routes
from services.api.routes import reporting_financials as reporting_financials_routes
from services.api.routes import investors as investors_routes
from services.api.routes import time_utils
from services.api.routes import my as my_routes
from services.api.routes import finance as finance_routes
from services.api.routes import tax as tax_routes
from services.api.routes import metrics as metrics_routes
from services.api.routes import quotes as quotes_routes
from services.api.routes import learning as learning_routes
from services.api.routes import learning_admin as learning_admin_routes  # NGI Learning Module (Sprint 1)
from services.api.routes import partners as partners_routes
# seed_entities_endpoint module not present; temporarily disable
# from services.api.routes import seed_entities_endpoint as seed_entities_routes
try:
    from services.api.auth_deps import require_admin as _require_admin_dep, require_clerk_user as _require_clerk_user_dep  # type: ignore
except Exception:
    _require_admin_dep = None  # type: ignore
    _require_clerk_user_dep = None  # type: ignore
from services.api.config import get_database_path, DATABASE_URL, SECRET_KEY, ALGORITHM
from sqlalchemy import text as sa_text
from services.api.database import get_db as get_session
from services.api.clerk_auth import verify_clerk_jwt
from jose import jwt as _jwt

# Configure logging (prefer stdout; add file handler if writable)
log_handlers = [logging.StreamHandler()]
try:
    os.makedirs('logs', exist_ok=True)
    log_handlers.append(logging.FileHandler('logs/api.log'))
except Exception:
    # In dev containers with bind mounts, file writes may be disallowed; ignore
    pass
...
def _db_connect():
    # Legacy helper; prefer SQLAlchemy Session from services.api.database
...
async def lifespan(app: FastAPI):
    ...
    # Start Mercury auto-sync scheduler (hourly)
    try:
        from services.api.scheduler import start_scheduler
        start_scheduler()
        logger.info("Mercury hourly auto-sync scheduler started")
    except Exception as e:
        logger.error(f"Failed to start Mercury scheduler: {e}")
    ...
    try:
        ...
        stop_scheduler()
        logger.info("Mercury scheduler stopped")
    except Exception as e:
...
async def add_security_headers(request: Request, call_next):
    ...
    # Security headers
    # Allow document viewer to be embedded in iframe for same origin
    if "/documents/view/" in str(request.url.path):
...
def require_partner_access():
    ...
    async def _require_partner(request: Request, credentials: HTTPAuthorizationCredentials = Depends(security)):
        ...
        if token:
            ...
            if not claims:
                try:
                    from services.api.clerk_auth import verify_clerk_session_cookie as _vsc
        ...
        if not credentials and request is not None:
            try:
                ...
                sess = None
            if sess:
                from services.api.clerk_auth import verify_clerk_session_cookie
...
async def get_current_partner_info(request: Request, credentials: HTTPAuthorizationCredentials = Depends(security)):
    ...
    if auth.lower().startswith('bearer '):
        try:
            from services.api.auth_deps import require_clerk_user as _rcu  # type: ignore
...
if _OPEN_NON_ACCOUNTING:
    ...
    # Legacy banking router removed; using bank reconciliation routes
    # app.include_router(documents.router)
    # Removed legacy financial_reporting router; use accounting_financial_reporting_routes below
    ...
    # Legacy aggregated accounting router not included; use scoped accounting routers below
else:
...
if _os_main.getenv('DISABLE_ACCOUNTING_GUARD') == '1':
    # Legacy aggregated accounting router removed; include only scoped accounting sub-routers
    ...
    from .routes import admin_cleanup
    ...
    # Legacy banking routers removed
...
# app.include_router(seed_entities_routes.router)
...
from services.api.routes import learning_content as learning_content_routes
from services.api.routes import learning_animations as learning_animations_routes
from services.api.routes import learning_enhanced as learning_enhanced_routes
