...
export default function AdvisoryProjectsPage() {
  ...
  const onSave = async (publish = false) => {
      ...
      if (publish) {
        ...
        if (!lenOk(name, 4, 120)) return void toast.error('Project name must be 4GÇô120 chars to publish')
        if (!lenOk(client, 2, 120)) return void toast.error('Client name must be 2GÇô120 chars to publish')
        if (!lenOk(summary, 20, 200)) return void toast.error('Summary must be 20GÇô200 chars to publish')
        if (!lenOk(description, 50, 4000)) return void toast.error('Description must be 50GÇô4000 chars to publish')
        if (!Number.isFinite(teamSize) || teamSize < 1) return void toast.error('Team size must be GëÑ 1 to publish')
        if (!Number.isFinite(hours) || hours < 1 || hours > 40) return void toast.error('Hours/week must be 1GÇô40 to publish')
        if (!Number.isFinite(duration) || duration < 1) return void toast.error('Duration (weeks) must be GëÑ 1 to publish')
  ...
  if (loading || authCheckLoading) {
    return (<div className="p-6">LoadingGÇª</div>)
  }
  if (!allowed) {
    return (
      <div className="p-6">
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {/* List */}
      {listLoading ? (
        <div className="text-sm text-muted-foreground">Loading projectsGÇª</div>
      ) : (
    ...
    >
      <div className="flex items-center p-3 md:p-4 gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-2 mt-2">
            ...
            {(p.start_date || p.end_date) ? (
              <span className="text-xxs px-2 py-0.5 rounded border">
                {(p.start_date || '').slice(0,10)}{p.end_date ? ' GåÆ ' + String(p.end_date).slice(0,10) : ''}
              </span>
            ) : null}
          </div>
          {/* Logos or badges */}
          ...
          {expanded && (
            <div className="mt-3 text-sm text-foreground/90">{p.description || 'GÇö'}</div>
          )}
      ...
      <div className="flex flex-wrap gap-2 mb-2">
        {value.map(m => (
          <span key={m} className="inline-flex items-center gap-1 px-2 py-1 text-xs rounded-full bg-muted border">
            {m}
            <button className="text-muted-foreground hover:text-foreground" onClick={() => remove(m)}>+ù</button>
          </span>
        ))}
      </div>
      <input
