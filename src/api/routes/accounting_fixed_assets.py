...
from src.api.services.depreciation_service import DepreciationService
from src.api.services.fixed_asset_detector import FixedAssetDetector
from src.api.services.audit_package_generator import AuditPackageGenerator
...
@router.post("/detect-asset")
async def detect_asset_from_document(
    document_id: int,
    extracted_data: dict,
    entity_id: int,
    db: AsyncSession = Depends(get_async_db)
):
    """Detect if a document represents a fixed asset purchase"""
    asset = await FixedAssetDetector.detect_fixed_asset(
        extracted_data, entity_id, document_id, db
    )
    ...
    if asset:
        return {
            ...
            "asset_name": asset.asset_name,
            "asset_category": asset.asset_category,
            "confidence": float(asset.detection_confidence),
            "message": "Fixed asset auto-detected and created in draft"
        }
    else:
        return {
            "detected": False,
            "message": "No fixed asset detected (below threshold or no matching keywords)"
        }
