...
async def generate_package(
    ...
    try:
        ...
        package = LearningPackage(
            ...
            generated_by_user_id=user.get("id") or user.get("email")
        )
        db.add(package)
        db.commit()
        db.refresh(package)
...
async def generate_feedback(
    ...
    """
    Generate AI feedback for a submission using GPT-5.
    Submission must pass validation before feedback can be generated.
    """
    from ..learning.ai_coach import generate_feedback_for_submission
    ...
    if submission.user_id != user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="You can only request feedback for your own submissions"
        )
    ...
    if result['status'] == 'validation_failed':
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Submission must pass validation before feedback can be generated. Fix errors and resubmit."
        )
