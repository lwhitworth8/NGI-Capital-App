...
async def preview(
    ...
    if type == 'IS':
        data = await _is(entity_id=entity_id, start_date=start, end_date=end, current_user=None, db=db)  # type: ignore
    ...
    if type == 'BS':
        data = await _bs(entity_id=entity_id, as_of_date=end, current_user=None, db=db)  # type: ignore
...
async def export_financials(
    ...
    bsd = await _bs(entity_id=entity_id, as_of_date=end, current_user=None, db=db)  # type: ignore
    cfd = await _cf(entity_id=entity_id, start_date=start, end_date=end, current_user=None, db=db)  # type: ignore
    if format == 'xlsx':
